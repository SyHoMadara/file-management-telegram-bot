...
from apps.telegram_bot.utils.utils import create_user_if_not_exists, get_user
...
async def _download_file(client, file_properties: File):
    ...
    try:
        ...
        await file_properties.download_message.edit_text("Error downloading file")
    except SaveFileException as e:
        logger.error(f"Error saving file {file_properties.file_name}: {str(e)}")
        await file_properties.download_message.edit_text("Error saving file")
    except FileTempException as e:
        logger.error(
            f"Error with temporary file for {file_properties.file_name}: {str(e)}"
        )
        await file_properties.download_message.edit_text("Error saving file")
...
async def _download_file_to_temp(client: Client, file_properties: File, temp_file):
    try:
        await client.download_media(
            file_properties.user_message, file_name=temp_file.name
        )
        logger.info(
            f"{file_properties.file_name} downloaded successfully to {temp_file.name}"
        )
...
async def _save_file_to_db(file_properties: File, tempfile):
    try:
        saved_file = save_file_to_db_task.delay(
            file_properties=file_properties, temp_file_path=tempfile
        )
        while not saved_file.ready():
            await asyncio.sleep(1)
        saved_file = saved_file.get()
        logger.info(
            f"File {file_properties.file_name} saved to database with ID {saved_file.id}"
        )
        return saved_file
    except SaveFileException as e:
        logger.error(
...
async def _finalize_download(file_properties: File, saved_file: FileManager):
    try:
        ...
        await sync_to_async(file_properties.user.save)(
            update_fields=["remaining_download_size"]
        )
        full_url = saved_file.file.url
        parsed_url = urlsplit(full_url)
        relative_path = parsed_url.path.lstrip("/") + "?" + parsed_url.query
        logger.info(
            f"File {file_properties.file_name} downloaded successfully. Remaining size: {file_properties.user.remaining_download_size}MB"
        )
        await file_properties.download_message.edit_text(
        ...
        logger.error(
            f"Error finalizing download for {file_properties.file_name}: {str(e)}"
        )
        await file_properties.download_message.edit_text("Error finalizing download.")
